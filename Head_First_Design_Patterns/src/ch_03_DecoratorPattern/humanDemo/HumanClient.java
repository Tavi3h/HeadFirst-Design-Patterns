...
public class HumanClient {
	...
	public static void main(String[] args) {
		Human hum = new Person();
		Decorator dec = new Decorator_two(new Decorator_one(new Decorator_zero(hum)));
		dec.wearClothes();
		dec.walkToWhere();
	}
}
...
所以当调用dt.wearClothes();dt.walkToWhere()的时候，因为每个具体装饰者类中都先调用super.wearClothes和super.walkToWhere()方法，
而该super已经由构造传递并指向了具体的某一个装饰者类（这个可以根据需要调换顺序），那么调用的即为装饰类的方法，然后才调用自身的装饰方法，即表现出一种装饰、链式的类似于过滤的行为。
3、具体被装饰者类，可以定义初始的状态或者初始的自己的装饰，后面的装饰行为都在此基础上一步一步进行点缀、装饰。
4、装饰者模式的设计原则为：对扩展开放、对修改关闭。
这句话体现在我如果想扩展被装饰者类的行为，无须修改装饰者抽象类，只需继承装饰者抽象类，实现额外的一些装饰或者行为即可对被装饰者进行包装。
